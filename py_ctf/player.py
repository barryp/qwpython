...
def player_stand1(*qwp_extra):
    ...
    if qc.self.weapon == defs.IT_AXE or qc.self.weapon == defs.IT_GRAPPLE:
...
def player_chain1(*qwp_extra):
    qc.self.frame = frame['axattd1']
    qc.self.nextthink = qc.time + 0.1
    qc.self.think = player_chain2
    qc.self.weaponframe = 2
    grapple.Throw_Grapple()


def player_chain2(*qwp_extra):
    qc.self.frame = frame['axattd2']
    qc.self.nextthink = qc.time + 0.1
    qc.self.think = player_chain3
    qc.self.weaponframe = 3


def player_chain3(*qwp_extra):
    qc.self.frame = frame['axattd3']
    qc.self.nextthink = qc.time + 0.1
    qc.self.think = player_chain3
    qc.self.weaponframe = 3
    if not qc.self.hook_out:
        player_chain5()
        return 
        
    if qc.self.velocity.length() >= 750:
        player_chain4()
        return 
        
    
def player_chain4(*qwp_extra):
    qc.self.frame = frame['deathc4']
    qc.self.nextthink = qc.time + 0.1
    qc.self.think = player_chain4
    qc.self.weaponframe = 4
    if not qc.self.hook_out:
        player_chain5()
        return 
...
death_sound_list = ('player/death1.wav', 
                    'player/death2.wav', 
                    'player/death3.wav', 
                    'player/death4.wav', 
                    'player/death5.wav')
...
def DeathSound(*qwp_extra):
    ...
    qc.self.noise = random.choice(death_sound_list) 
...
def GibPlayer(*qwp_extra):
    ...
    qc.self.deadflag = defs.DEAD_DEAD
    if defs.damage_attacker.classname == 'teledeath':
        qc.self.staydeadtime = qc.time + 2 + (random.random() * 3)
        qc.self.sound(defs.CHAN_VOICE, 'player/teledth1.wav', 1, defs.ATTN_NONE)
        return 
        
    if defs.damage_attacker.classname == 'teledeath2':
        qc.self.staydeadtime = qc.time + 3 + (random.random() * 6)
        qc.self.sound(defs.CHAN_VOICE, 'player/teledth1.wav', 1, defs.ATTN_NONE)
        return 
