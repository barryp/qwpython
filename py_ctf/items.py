...
def item_megahealth_rot(*qwp_extra):
    ...
    if qc.other.health > qc.other.max_health and not (qc.other.player_flag & defs.ITEM_RUNE4_FLAG):
        qc.other.health -= 1
    ...
    if defs.deathmatch == 1 or defs.deathmatch == 3: #  deathmatch 2 is silly old rules
        qc.self.nextthink = qc.time + 20
        qc.self.think = SUB_regen
...
def armor_touch(*qwp_extra):
    ...
    if qc.self.classname == 'item_armor1':
        ...
        value = 100
        bit = defs.IT_ARMOR1        
    elif qc.self.classname == 'item_armor2':
        type = 0.6
        value = 150
        bit = defs.IT_ARMOR2        
    elif qc.self.classname == 'item_armorInv':
        type = 0.8
        value = 200
        bit = defs.IT_ARMOR3
    else:
...
def Deathmatch_Weapon(old, new, *qwp_extra):
    if qc.self.weapon == defs.IT_GRAPPLE and qc.self.button0:
        return  #  never change if we pulled ourselves to it
        
    #  change self.weapon if desired
    if RankForWeapon(new) < RankForWeapon(qc.self.weapon):
        qc.self.weapon = new
...
def powerup_touch(*qwp_extra):
    ...
    elif qc.self.classname == 'item_artifact_invulnerability':
        qc.other.invincible_time = 1
        qc.other.invincible_finished = qc.time + 30        
    elif qc.self.classname == 'item_artifact_invisibility':
        qc.other.invisible_time = 1
        qc.other.invisible_finished = qc.time + 30        
    elif qc.self.classname == 'item_artifact_super_damage':
...
def RuneRespawn(*qwp_extra):
    oself = qc.self
    #  choose random starting points
    qc.self = SelectRuneSpawnPoint()
...
# ===============
# 
def DropRune(*qwp_extra):
    if qc.self.player_flag & defs.ITEM_RUNE1_FLAG:
        Do_DropRune(defs.ITEM_RUNE1_FLAG)
    if qc.self.player_flag & defs.ITEM_RUNE2_FLAG:
        Do_DropRune(defs.ITEM_RUNE2_FLAG)
    if qc.self.player_flag & defs.ITEM_RUNE3_FLAG:
        Do_DropRune(defs.ITEM_RUNE3_FLAG)
    if qc.self.player_flag & defs.ITEM_RUNE4_FLAG:
        Do_DropRune(defs.ITEM_RUNE4_FLAG)
